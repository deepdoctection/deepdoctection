...
_NAME = "testlayout"
_DESCRIPTION = "A small test set consisting of a subset of six images with layout annotations. " \
               "The set is meant for debugging purposes and nothing else"

_LICENSE = "The annotations in this dataset belong to Dr. Janis Meyer and are licensed" \
...
_URL = ["https://www.googleapis.com/drive/v3/files/1ZD4Ef4gd2FIfp7vR8jbnrZeXD3gSWNqE?alt"
...
_ANNOTATION_FILES: Dict[str, Union[str, List[str]]] \
    = {"test": "xrf_layout_test.jsonl", "predict": "xrf_layout_test_predict.jsonl"}
...
class LayoutTestBuilder(DataFlowBaseBuilder):
    ...
    def build(self, **kwargs: Union[str, int]) -> DataFlow:
        ...
        # Load
        path = os.path.join(self.get_workdir(), os.path.join(self.splits[split],
        ...
        prodigy_mapper = prodigy_to_image(  # type: ignore # pylint: disable=E1120
            categories_name_as_key=self.categories.get_categories(  # type: ignore
                init=True, name_as_key=True),
            load_image=load_image, fake_score=fake_score)  # pylint: disable=E1120  # 259 # type: ignore
