...
_NAME = "publaynet"
_DESCRIPTION = "PubLayNet is a dataset for document layout analysis. It contains images of research papers and " \
               "articles" \
...
_URL = "https://dax-cdn.cdn.appdomain.cloud/dax-publaynet/1.0.0/" \
...
class PublaynetBuilder(DataFlowBaseBuilder):
    ...
    def build(self, **kwargs: Union[str, int]) -> DataFlow:  # pylint: disable=W0221 (#3812)   type: ignore
        ...
        # Map
        df = MapDataComponent(df, lambda dp: self.get_workdir() + self.location + "/" +
                                             self.get_split(split) + "/" + dp, "file_name")
        coco_mapper = coco_to_image(self.categories.get_categories(init=True), load_image,
        ...
        if self.categories.is_filtered():
            df = MapData(df, filter_cat(self.categories.get_categories(as_dict=False, filtered=True),
                                        # type: ignore  # pylint: disable=E1120
