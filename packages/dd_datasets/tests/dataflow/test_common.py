...
from dd_datasets.dataflow.common import (
    BatchData,
    ConcatData,
    FlattenData,
    JoinData,
...
def test_flatten_data_with_dicts(dict_list_for_flatten: List[Dict[str, Any]]) -> None:
    """
    Test FlattenData flattens dict elements correctly
    """
    # Arrange
    df_base = DataFromList(dict_list_for_flatten, shuffle=False)
    df = FlattenData(df_base)
    ...
    # Assert
    assert len(output) == 2
    assert output[0] == {"a": 1, "b": 2}
    assert output[1] == {"c": 3, "d": 4}
