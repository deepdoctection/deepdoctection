...
from dd_core.utils.object_types import LayoutLabel, ObjectTypes, TableKey
...
def test_pub_to_image_basic(
    ...
    cells = img.get_annotation(category_names=LayoutLabel.CELL)
    assert len(cells) == _count_cells_with_bbox(pubtabnet_datapoint)
    summary = img.summary
    assert summary.get_sub_category(TableKey.NUMBER_OF_ROWS).category_id == 14
    assert summary.get_sub_category(TableKey.NUMBER_OF_COLUMNS).category_id == 9
...
def test_pub_to_image_rows_cols_items(
    ...
    assert img is not None
    summary = img.summary
    n_rows = summary.get_sub_category(TableKey.NUMBER_OF_ROWS).category_id
    n_cols = summary.get_sub_category(TableKey.NUMBER_OF_COLUMNS).category_id
    items = img.get_annotation(category_names=TableKey.ITEM)
...
def test_pub_to_image_dd_pipe_like(
    ...
    img = mapper(pubtabnet_datapoint)
    assert img is not None
    tables = img.get_annotation(category_names=LayoutLabel.TABLE)
    assert len(tables) == 1
    table = tables[0]
    assert table.image is not None
    cells = img.get_annotation(category_names=LayoutLabel.CELL)
    words = img.get_annotation(category_names=LayoutLabel.WORD)
