...
from dd_core.utils.object_types import LanguageCode, LayoutLabel
...
def test_tesseract_ocr_predict_words_basic(monkeypatch: pytest.MonkeyPatch, sample_np_img: NDArray[Any]) -> None:
    ...
    assert all(r.class_name == LayoutLabel.WORD for r in results)
...
def test_tesseract_ocr_predict_lines_enabled(monkeypatch: pytest.MonkeyPatch, sample_np_img: NDArray[Any]) -> None:
    ...
    assert any(r.class_name == LayoutLabel.LINE for r in results)
    line_texts = [r.text.lower() for r in results if r.class_name == LayoutLabel.LINE and r.text is not None]
...
def test_tesseract_ocr_set_language_mapping() -> None:
    ...
    det.set_language(LanguageCode.GERMAN)  # uses `ObjectTypes` enum path; `nn` -> 'eng'
