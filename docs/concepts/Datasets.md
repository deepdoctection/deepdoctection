...
If a dataset has been generated by running a pipeline, it will not be a folder of annotation files but a folder of
saved [`Document`][dd_core.doc.Document]s. [`DocumentDatasetFactory`][dd_datasets.doc_factory.DocumentDatasetFactory]
builds a dataset streaming such a folder, provided it has the layout
...
`build(mode="doc")` yields `Document` datapoints, `build(mode="image")` yields the `Image` of every page in page
order.

`category_id`s serialized by a pipeline are often `DEFAULT_CATEGORY_ID`. As evaluation and training compare
`category_id`s and not `category_name`s, the ids have to be derived from the categories of the dataset. Which ids
have to be assigned depends on the `dataset_type`, which is an attribute of the dataset and not of the dataflow
builder, hence the factory:

```python
dataset = dd.DocumentDatasetFactory.make(name="my_docs",
                                         location="my_docs",
                                         dataset_type=dd.DatasetKind.object_detection,
                                         init_categories=[dd.LayoutLabel.TEXT, dd.LayoutLabel.TABLE])

df = dataset.dataflow.build(mode="image", load_image=True) # (1)
```

1. Every `ImageAnnotation` with `category_name` 'text' or 'table' now carries `category_id` 1 or 2. Note, that for
   `object_detection` and `token_classification` all annotations with a `category_name` that is not listed in
   `init_categories` will be **removed** from the datapoint.

`mode="doc"` only aligns ids for `dataset_type=dd.DatasetKind.sequence_classification`, where the label sits in
`document.summary` under `DocumentKey.DOCUMENT_TYPE`. For every other dataset type use `mode="image"`.
