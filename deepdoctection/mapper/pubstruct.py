...
from ..utils.settings import CellType, LayoutType, Relationships, TableType, WordType, SummaryType, ObjectTypes
...
def _add_items(image: Image, item_type: str, categories_name_as_key: dict[ObjectTypes, int],
               pubtables_like: bool) -> Image:
    ...
    for item_num in range(1, number_of_items + 1):
        cell_item = list(
            filter(
                lambda x: x.get_sub_category(item_number).category_id == item_num, cells  # pylint: disable=W0640
            )
        )
