...
"""
This module defines all configuration options for the deepdoctection analyzer pipeline using an AttrDict structure.
...
## General Configuration
    ...
    LANGUAGE: Relevant only for Tesseract OCR. Specifies the language model to use.
        Supported language codes are listed at:
        https://tesseract-ocr.github.io/tessdoc/Data-Files-in-different-versions.html.
        Note: models must be downloaded in advance.

    LIB: Deep learning framework. Choose either 'TF' (TensorFlow) or 'PT' (PyTorch).
...
## Pipeline Component Activation
    ...
    USE_TABLE_SEGMENTATION: Enables table segmentation (third and later pipeline components).
    ...
    USE_TABLE_REFINEMENT: Enables optional refinement of table structure to ensure valid HTML generation.
        Should be set to False when using the Table Transformer approach.

    USE_PDF_MINER: Enables text extraction using PDFPlumber. Only works on PDFs with embedded text layers.
        Configure additional behavior using PDF_MINER.*

    USE_OCR: Enables OCR functionality using Tesseract, DocTr, or Textract.
    ...
    USE_LAYOUT_LINK: Enables MatchingService to associate nearby layout elements (e.g., figures and captions).

    USE_LINE_MATCHER: Enables line matching in post-processing. Useful when synthetic line elements are created
...
### TensorFlow Layout Configuration
    ...
    TF.LAYOUT.WEIGHTS: Relevant when LIB = TF. Specifies the layout detection model.
        This model should detect multiple or single objects across an entire page.
        Currently, only one default model is supported.

    TF.LAYOUT.FILTER: Filters out unnecessary categories from the layout detection model output.
...
### PyTorch Layout Configuration
    ...
    PT.ENFORCE_WEIGHTS.LAYOUT: Relevant when LIB = PT. Allows selection between two model formats:
    ...
    PT.LAYOUT.WEIGHTS: Specifies the PyTorch layout detection model (standard weights).
    ...
    PT.LAYOUT.WEIGHTS_TS: Specifies the TorchScript version of the layout model.
        Must detect single or multiple objects across the full page.
        Acceptable format: .ts files (e.g., layout/d2_model_0829999_layout_inf_only.ts).

    PT.LAYOUT.FILTER: Filters out unwanted categories from the model's predictions.
        Accepts either string values (e.g., ['list', 'figure']) or ObjectTypes
        (e.g., [LayoutType.LIST, LayoutType.FIGURE]).

    PT.LAYOUT.PADDING: Adds padding to the image, which may be required for some models such as
    ...
    PT.LAYOUT.PAD.*: Padding values for each edge of the image (TOP, RIGHT, BOTTOM, LEFT).

## Layout NMS Configuration

    LAYOUT_NMS_PAIRS.*: Non-Maximum Suppression (NMS) configuration for overlapping layout elements.
...
### TensorFlow Item (Row/Column) Detection
    ...
    TF.ITEM.WEIGHTS: Relevant when LIB = TF. Specifies the item detection model (for rows and columns).
        Currently, only the default model is supported.

    TF.ITEM.FILTER: Filters out unnecessary categories from the item detection model.
        Accepts either a list of strings (e.g., ['row', 'column']) or ObjectTypes.

### PyTorch Item (Row/Column) Detection

    PT.ENFORCE_WEIGHTS.ITEM: Relevant when LIB = PT. Use either TorchScript weights via PT.ITEM.WEIGHTS_TS
    ...
    PT.ITEM.WEIGHTS: Specifies the PyTorch model weights for item detection.
        Use either .pt or .safetensors files.

    PT.ITEM.WEIGHTS_TS: Specifies the TorchScript model for item detection.
        Use .ts files for deployment without model implementation dependencies.

    PT.ITEM.FILTER: Filters out unnecessary categories from the item detection model.
    ...
    PT.ITEM.PADDING: Enables image padding for item detection. Required for models such as
    ...
    PT.ITEM.PAD.*: Padding values for each edge of the sub-image (TOP, RIGHT, BOTTOM, LEFT).
...
### TensorFlow Cell Detection
    ...
    TF.CELL.WEIGHTS: Configuration for the second SubImagePipelineComponent.
        This is only used in the original Deepdoctection table recognition approach,
        not with the Table Transformer method.
        The CELL configuration structure mirrors that of the ITEM component.

    TF.CELL.FILTER: Filters out unnecessary categories from the cell detection model output.
...
### PyTorch Cell Detection
    ...
    PT.CELL.FILTER: Filters out unwanted categories from the cell detection model.

    PT.CELL.PADDING: Enables padding for the sub-image used in cell detection.
    ...
    PT.CELL.PAD.*: Padding values for each edge of the sub-image used in cell detection (TOP, RIGHT, BOTTOM, LEFT).

## Table Segmentation Configuration

    SEGMENTATION.ASSIGNMENT_RULE: Specifies the rule used to assign detected cells to rows and columns.
    ...
    SEGMENTATION.REMOVE_IOU_THRESHOLD_COLS: Same as above, but applied to columns.

    SEGMENTATION.FULL_TABLE_TILING: Ensures that predicted rows and columns fully cover the table region.
    ...
    SEGMENTATION.STRETCH_RULE: Defines how row and column boundaries are stretched when tiling is enabled.
        Options:
        - "left": lower edge equals the upper edge of the next row
        - "equal": lower edge is halfway between two adjacent rows

    SEGMENTATION.TABLE_NAME: Specifies the layout category used to identify tables.
    ...
    SEGMENTATION.ITEM_NAMES: Lists the layout categories used to identify row and column elements.
...
https://github.com/jsvine/pdfplumber/blob/main/pdfplumber/utils/text.py
    ...
    PDF_MINER.X_TOLERANCE: Horizontal tolerance when merging characters into words.
        Characters that are horizontally closer than this value will be grouped into a single word.

    PDF_MINER.Y_TOLERANCE: Vertical tolerance when grouping characters into lines.
...
All other engines must be set to False.
    ...
    OCR.USE_TESSERACT: Enables Tesseract as the OCR engine.
    ...
    OCR.CONFIG.TESSERACT: Path to the Tesseract configuration file.

    OCR.USE_DOCTR: Enables DocTR as the OCR engine.
        DocTR provides flexible and lightweight OCR models with strong accuracy and versatility.

    OCR.USE_TEXTRACT: Enables AWS Textract as the OCR engine.
    ...
    OCR.WEIGHTS.DOCTR_WORD.TF: TensorFlow weights for the word detection model used by DocTR.

    OCR.WEIGHTS.DOCTR_WORD.PT: PyTorch weights for the word detection model used by DocTR.

    OCR.WEIGHTS.DOCTR_RECOGNITION.TF: TensorFlow weights for the text recognition model used by DocTR.

    OCR.WEIGHTS.DOCTR_RECOGNITION.PT: PyTorch weights for the text recognition model used by DocTR.

## Text Processing Configuration

    TEXT_CONTAINER: Specifies the annotation type used as a text container.
    ...
    WORD_MATCHING.MAX_PARENT_ONLY: If a text container overlaps with multiple layout sections,
...
The distance is calculated using the center points of the layout elements.
    ...
    LAYOUT_LINK.PARENTAL_CATEGORIES: Specifies the parent layout categories in the link relationship.
        These are the elements to which related components (e.g., captions) should be linked.

    LAYOUT_LINK.CHILD_CATEGORIES: Specifies the child layout categories in the link relationship.
