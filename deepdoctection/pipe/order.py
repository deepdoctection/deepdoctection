...
from ..datapoint.box import merge_boxes, BoundingBox
from ..datapoint.image import Image
from ..extern.base import DetectionResult
from ..extern.tp.tpfrcnn.utils.np_box_ops import ioa as np_ioa
from ..utils.settings import LayoutType, Relationships, TypeOrStr, ObjectTypes,get_type
...
class OrderGenerator:
    ...
    @staticmethod
    def group_words_into_lines(word_anns: Sequence[ImageAnnotation], image_id: Optional[str] = None) -> List[Tuple[int, int, str]]:
        ...
        rows_dict = {
            idx: key[0] for idx, key in enumerate(sorted(rows_dict.items(), key=lambda it: it[1]["upper"]))
            # type:ignore
        }
    ...
    @staticmethod
    def group_lines_into_lines(line_anns: Sequence[ImageAnnotation], image_id: Optional[str] = None) -> List[Tuple[int, int, str]]:
        reading_lines = []
        ...
        for idx, col in enumerate(columns):
            ...
            if not component_found:
                connected_components.append(
                    {"top": col.uly, "bottom": col.lry, "left": col.ulx, "column": [col_dict]}
                )
    ...
    def order_blocks(self, anns: List[ImageAnnotation],  image_width: float, image_height: float, image_id: Optional[str] = None):
        reading_blocks = []
        columns: List[BoundingBox] = []
        anns.sort(key=lambda x: (x.bounding_box.transform(image_width, image_height).cy,
        ...
        for ann in anns:
            ...
            for idx, col in enumerate(columns):
                ...
                if (first_condition and (fourth_condition or fifth_condition)) or (  # pylint: disable=R0916
                        second_condition and (fourth_condition or fifth_condition) or (
                        third_condition and (fourth_condition or fifth_condition)
                )
                ):
            ...
            if not column_found:
                columns.append(
                        BoundingBox(absolute_coords=False,
        ...
        for idx in range(max_block_number+1):
            filtered_blocks = list(filter(lambda x: x[0]==idx, blocks))
    ...
    def _consolidate_columns(self, columns):
        ...
        column_dict= {hor: ver for hor, ver in zip(child_index, parent_index)}
        counter = 0
        for idx, _ in enumerate(columns):
    ...
    def _sort_anns_grouped_by_blocks(block, anns, image_width, image_height):
        ...
        block_anns.sort(key=lambda x: (round(x.bounding_box.transform(image_width, image_height).uly,2),
...
class TextLineGenerator:
    ...
    def __init__(self,
                 make_sub_lines: bool,
                 line_category_id: Union[int,str],
    ...
    def _make_detect_result(self, box, relationships):
        return DetectionResult(box=box.to_list(mode="xyxy"),
                            class_name=LayoutType.line,
                            class_id=self.line_category_id,
                            absolute_coords=box.absolute_coords,
                            relationships=relationships)

    def create_detection_result(self, word_anns: Sequence[ImageAnnotation], image_width: float, image_height: float, image_id: Optional[str] = None):
        if not word_anns:
            return []
        ...
        for row in range(1, number_rows + 1):
            ...
            if len(anns_per_row) >= 2 or not self.make_sub_lines:
                ...
                merge_box = merge_boxes(*boxes)
                detection_result = self._make_detect_result(merge_box,
                                                            {"child": [ann.annotation_id for ann in anns_per_row]})
                detection_result_list.append(detection_result)
...
class TextOrderService(PipelineComponent):
    ...
    def __init__(self, text_container: str,
        ...
        :param text_block_categories: name of image annotation that have a relation with text containers (or which might be
                                 text containers themselves).
        :param floating_text_block_categories: name of image annotation that belong to floating text. These annotations form
        ...
        self.text_line_generator = TextLineGenerator(self.include_residual_text_container, line_category_id, paragraph_break)
    ...
    def serve(self, dp: Image) -> None:
        ...
        if self.include_residual_text_container :
            ...
            residual_text_container_anns = [ann for ann in text_container_anns if ann.annotation_id not in mapped_text_container_ids]
    ...
    def _create_lines_for_words(self, word_anns: Sequence[ImageAnnotation]) -> Sequence[ImageAnnotation]:
        detection_result_list = self.text_line_generator.create_detection_result(word_anns,
    ...
    def order_text_in_text_block(self, text_block_ann: ImageAnnotation):
        ...
        if self.text_container == LayoutType.word:
            word_order_list = self.order_generator.group_words_into_lines(text_container_ann,
                                                        self.dp_manager.datapoint.image_id)
        else:
            word_order_list = self.order_generator.group_lines_into_lines(text_container_ann,
                                                        self.dp_manager.datapoint.image_id)
    ...
    def order_blocks(self, text_block_anns: List[ImageAnnotation]):
        block_order_list = self.order_generator.order_blocks(text_block_anns,
                                          self.dp_manager.datapoint.width,
                                          self.dp_manager.datapoint.height)
    ...
    def clone(self) -> PipelineComponent:
        return self.__class__(copy(self.text_container),
