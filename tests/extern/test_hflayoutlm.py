...
def get_token_class_results(uuids: List[str], input_ids, attention_mask,
                            token_type_ids, boxes, tokens,
                            model) -> List[TokenClassResult]:
...
class TestHFLayoutLmTokenClassifier:
    ...
    def test_categories_are_constructed_properly() -> None:
        ...
        # Arrange
        categories_explicit = ["FOO","BAK","O"]

        # Act
        model = HFLayoutLmTokenClassifier(categories_explicit=categories_explicit)
    ...
    @mark.requires_pt
    @patch("deep_doctection.extern.hflayoutlm.LayoutLMForTokenClassification.from_pretrained", MagicMock())
    @patch("deep_doctection.extern.hflayoutlm.predict_token_classes",
           MagicMock(side_effect=get_token_class_results))
    def test_hf_layout_lm_predicts_token(layoutlm_input: JsonDict, categories_semantics: List[str],
                                         categories_bio: List[str],token_class_names: List[str]) -> None:
        """
